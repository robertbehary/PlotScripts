...
// plot data from Cosmic data text files in the format FTCalCosmic_*****_
// when given a directory where all of these text files are located.
//
// Specifically this script makes three plots of:
// 1.) component# vs charge mean
// 2.) component# vs ratio of charge mean of a run/earliest run in the directory
...
class Run {
  ...
  Double_t sector[332],
           ...
           chargeMean[332],
           chargeSigma[332],
           chiSquared[332]; // variables from the text file all here for future plots if desirable
...
public:
  ...
  Run(const string filename) {
    ...
    int pos = filename.find("FTCalCosmic_");
    runNumber = filename.substr(pos+12,5);
  ...
  TGraphErrors chargeMeanGraph(int total, int color) {
    ...
    auto graph = new TGraphErrors(num, component, chargeMean, nullptr,nullptr);
  ...
  void setRatio(Run* lowest) {
    for (int i = 0; i<num; i++) {
      this->ratio[i] = this->chargeMean[i] / lowest->chargeMean[i];
    }
  }

  // method that returns a TGraph of the Component Number vs. the Mean Amplitude Ratio
  TGraphErrors chargeMeanRatioGraph(Run* lowest, int total, int color) {
  ...
  TGraphErrors radialDistanceGraph(Run* lowest) {
    ...
    graph -> SetDrawOption("AP");
    graph -> SetTitle(graphTitle);
    graph->SetMinimum(0.7); // setting range for y-axis becuase ratios may be off
    graph->SetMaximum(1.3); // adjust these as needed for averages
    graph->GetXaxis()->SetTitle("Radial Distance [arb. units]");
    graph->GetYaxis()->SetTitle("Charge Mean Ratio Average");
...
void Cosmicanalysis() {
  ...
  string path = "/Users/robertbehary/Genoa/clas12calibration-ft/OriginalCosmicData/";
  char directory[] = "/Users/robertbehary/Genoa/clas12calibration-ft/OriginalCosmicData";
  ...
  // loop to make charge mean plot
  auto chargeMeanPlot = new TCanvas("chargeMeanPlot","chargeMeanPlot",600,400);
  TGraph *g[i];
  TMultiGraph *mg = new TMultiGraph();
  for (int j = 0; j < i; j++) {
    g[j] = (TGraphErrors*)(collection[j] -> chargeMeanGraph(i,j+1)).Clone();
    mg -> Add(g[j]);
  }
  mg->SetTitle("Charge Mean vs. Component Number;Component Number;Charge Mean [pC]");
  mg->Draw("AP");
  mg->SetMinimum(0); // setting range for y-axis becuase ratios may be off
  mg->SetMaximum(10);
  chargeMeanPlot -> BuildLegend();
  cout << "Charge Mean Plot Finished." << endl;
  ...
  // loop to make charge mean ratio plot
  auto chargeMeanRatioPlot = new TCanvas("chargeMeanRatioPlot","chargeMeanRatioPlot",600,400);
  TGraph *g1[i];
  TMultiGraph *mg1 = new TMultiGraph();
  for (int j = 0; j < i; j++) {
    g1[j] = (TGraphErrors*)(collection[j] -> chargeMeanRatioGraph(lowest,i,j+1)).Clone();
    mg1 -> Add(g1[j]);
  }
  mg1->SetTitle("Charge Mean Ratio vs. Component Number;Component Number;Charge Mean Ratio");
  mg1->Draw("AP");
  mg1->SetMinimum(0.6); // setting range for y-axis becuase ratios may be off
  mg1->SetMaximum(1.3);
  chargeMeanRatioPlot -> BuildLegend();
  cout << "Charge Mean Ratio Plot Finished." << endl;
